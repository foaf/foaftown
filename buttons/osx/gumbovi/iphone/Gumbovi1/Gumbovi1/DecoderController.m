...
- (void) buttonDone:(id)sender {
	NSLog(@"DecoderController buttondone. %@ ", sender);	
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
- (void) loadView {
	NSLog(@"DecoderController loadView called.");
  [super loadView];
}
...
- (void) viewDidAppear:(BOOL)animated {
	NSLog(@"DecoderController viewDidAppear.");
    if( nil != _imagePicker ) {
    return;
  }
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)result {
  ...
  VerboseLog(@"DECODER DONE, trying to restore main app, %@", gad);
  // NSString *s = (NSString *)result.text;
  // NSString *s = (@"%@",s);
  // [gad newJID:s];
	[gad newJID:[result.text stringByReplacingOccurrencesOfString:@"xmpp:" withString:@""]];
  UITabBarController *tbc =  [gad tabBarController];
  VerboseLog(@"Got tbc: %@", tbc);
  UINavigationController *myQrController = [tbc.viewControllers objectAtIndex:4];
  VerboseLog(@"Got my qrc, asking it to pop: %@", myQrController);
